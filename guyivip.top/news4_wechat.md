...
作为一个技术宅，周末总想折腾点有意思的东西。最近迷上了树莓派这个小玩意，巴掌大的电脑却能跑各种AI模型，简直是我的理想玩具。看着网上各种用树莓派做智能摄像头、语音助手的项目，心里痒痒的。于是想着：能不能把视觉AI和语音识别结合起来，做一个真正能"看"又能"听"的智能设备？这样放在家里，既能当安防摄像头，又能跟它对话问它看到了什么，想想就觉得酷！于是就有了这个项目。

## 硬件准备

- **麦克风**：免驱动USB麦克风（笔者用的普通USB麦克风）
- **摄像头**：树莓派官方广角摄像头（用的Cam/Disp0接口）
- **软件环境**：Whisper语音识别 + Ollama LLaVA多模态模型

## 硬件测试

### 测试摄像头

首先连接摄像头，用的是树莓派Cam/Disp0接口。

**注意：** 这个接口可以先用指甲把黑色塑料片扣出来向上提出来一部分，然后把线插入后再复位塑料片，尽量不要硬塞（因为线会弯折，别问我怎么知道的）。
...
请留意关键的设备号：`card 2, device 0`这两个参数
...
**参数解释：**
- `-D plughw:2,0`：强制使用 "card 2，device 0" 的 USB 麦克风
- `-d 5`：录制时长 5 秒
- `-f cd`：采用 CD 级音质（16 位深度、44100Hz 采样率）
...
编译完成后，会在项目根目录生成一个名为whisper的可执行文件。可以用以下命令查看：
...
**注意：** 新版的whisper.cpp将可执行文件移到了 build/bin 目录下，且改名为 whisper-cli：
...
**注意：** 随着版本更新模型名称可能变化，如果树莓派下载太慢，可以电脑下载后传输。

项目链接：[https://huggingface.co/ggerganov/whisper.cpp/tree/main](https://huggingface.co/ggerganov/whisper.cpp/tree/main)

## 测试识别

如果已经安装了llava模型，那么可以进行音频+视频+AI项目的测试。

### 方式1: CLI控制台调用

这种方式相当于先使用whisper识别音频，然后将识别结果作为输入，调用llava模型进行视频分析。
...
def capture_image():
    ...
    try:
        result = subprocess.run(
            ["libcamera-still", "-o", IMAGE_FILE, "-t", "2000",
        ...
        if result.returncode != 0:
            ...
            return False
        if os.path.exists(IMAGE_FILE) and os.path.getsize(IMAGE_FILE) > 102400:
...
def image_to_base64(photo_path):
    ...
    try:
        ...
        expected_length = int(photo_size * 1.37)
